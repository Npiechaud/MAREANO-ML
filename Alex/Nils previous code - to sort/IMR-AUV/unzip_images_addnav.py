...
def main():
    ...
    # import navigation
    # import smoothed navigation
    smooth_nav = pd.read_csv(csv_directory + "/" + os.path.basename(directory) + "_smoothnav.csv")
    smooth_nav = smooth_nav[['timestamp', 'lat', 'lon', 'depth', 'datetime','pitch','roll','heading']]
    ...
    for zip in zips:
        ...
        for image in images:
            ...
            dt = datetime.strptime(s_new, "%Y:%m:%d %H:%M:%S.%f").replace(tzinfo=tz.UTC)
            ts = dt.timestamp()
        ...
        df_nav = pd.merge_asof(images_dt, smooth_nav, # [['timestamp', 'lat', 'lon', 'depth']]
                                   on='timestamp',
                                   direction="nearest",
                                   allow_exact_matches=False, suffixes=('_nav', '_image'))

        # export csv to the chunk's directory
        # df.to_csv(extract_dir + '/' + os.path.basename(extract_dir) + '.csv',index=False)
        #df_nav.to_csv(extract_dir + '/' + os.path.basename(extract_dir) + '.csv',index=False)
...
# add metadata - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
images_df = pd.read_csv(csv_directory + "/" + os.path.basename(directory) + "_images.csv")
...
# add the metadata from the navlab files
nav_images_hr = pd.merge_asof(images_df, smooth_nav[['timestamp','lat','lon','depth','pitch','roll','heading','datetime']], on='timestamp',
                            direction = "nearest",
                            allow_exact_matches=False,  suffixes=('_image','_nav' )   )

# add the altitude from the internal nav - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# import internal navigation
int_nav = pd.read_csv(csv_directory + "/internalnavandalt.csv" )
int_nav =  int_nav[[ 'Time', 'NAVIGATION_SYSTEM_DATA_NAV_LATITUDE',
       'NAVIGATION_SYSTEM_DATA_NAV_LONGITUDE',
       'NAVIGATION_SYSTEM_DATA_NAV_DEPTH','ALTITUDE_DATA_PRIMARY_ALT_ALTITUDE','datetime']]
int_nav.columns = ['timestamp', 'lat', 'lon', 'depth','altitude', 'datetime_internal']
# with another nearest match (altitude shoulnt change too much over 1s
nav_images = pd.merge_asof(nav_images_hr  ,
                           int_nav[['timestamp','altitude','datetime_internal']],
                           #nav_compare[['datetime_navlab','altitude','timestamp']],
              #  on='datetime_navlab',
              on = 'timestamp',
              direction="nearest",
              allow_exact_matches=False, suffixes=('image', 'internalNav'))


# does the datetime look ok compared to image names? -- - - - - - - - - - - - -
# are all the datetime columns the same
nav_images[['datetime_nav']]


nav_images.to_csv(csv_directory + "/image_navlabposition.csv", index = False)
